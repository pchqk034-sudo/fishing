...
(function () {
  ...
  const State = {
    ...
    switchUser(id) {
      ...
      this.save();                      // 現在の利用者を確定保存
      this.data.currentUser = id;
      this.hydrate(); this.save();
    },
    addUser(name) {
      const id = "u" + Date.now().toString(36);
      const u = this.blank(name);
      // APIキーは家族で共有できるよう引き継ぐ（各自で変更可）
      const cur = this.data.settings || {};
      u.settings.provider = cur.provider || "gemini";
      u.settings.model = cur.model;
      u.settings.keys = Object.assign({ gemini: "", anthropic: "" }, cur.keys || {});
      this.save();
    ...
    deleteUser(id) {
      if (Object.keys(this.data.users).length <= 1) return false;
      delete this.data.users[id];
      if (this.data.currentUser === id) this.data.currentUser = Object.keys(this.data.users)[0];
      this.hydrate(); this.save();
      return true;
    },
    log(date) {
  ...
  function render() {
    ...
    app.innerHTML = `
      <header class="topbar">
        ...
        <div class="tb-right">
          <button class="user-chip" id="user-btn">👤 ${escapeHtml(State.currentName())} ▾</button>
...
写真に写る料理ごとに1食分の推定値を入れてください。`;
  ...
  function renderSettings(view) {
    ...
    view.innerHTML = `
      ...
      <section class="card">
        <h3>👨‍👩‍👧 家族と使う</h3>
        <p class="muted">現在の利用者: <b>${escapeHtml(State.currentName())}</b>（登録 ${State.users().length}人）</p>
        <div class="row wrap">
          <button class="btn sm primary" id="set-users">使う人を切り替え / 追加</button>
          <button class="btn sm" id="set-share">アプリのURLをコピー</button>
        </div>
        <p class="muted"><b>それぞれのスマホで使う場合</b>（おすすめ）: 上のURLを家族に送り、各自のブラウザで開くだけです。
          データは各自の端末に保存され、混ざりません。<br>
          <b>1台を共有する場合</b>: 「使う人を切り替え」で家族を追加すると、画面右上から切り替えできます。</p>
    ...
    $("#set-users").onclick = () => openUserPicker();
    $("#set-share").onclick = () => {
  ...
  function openUserPicker() {
    const modal = makeModal("👤 使う人を選ぶ");
    const body = $(".modal-body", modal);
    const draw = () => {
      const users = State.users();
      body.innerHTML = `
        <p class="muted">1台を家族で共有する場合に使います。記録・プロフィール・設定は<b>人ごとに分かれて</b>保存されます。</p>
        <div class="fp-list">
          ${users.map((u) => `
            <button class="fp-item ${u.id === State.data.currentUser ? "sel" : ""}" data-uid="${u.id}">
              <span class="fp-name">${escapeHtml(u.name)}${u.id === State.data.currentUser ? " <span class=\"badge good sm\">選択中</span>" : ""}
                <small>${Object.keys(State.data.users[u.id].logs || {}).length}日分の記録</small></span>
              <span class="fp-k">切替</span>
            </button>`).join("")}
        </div>
        <div class="row wrap" style="margin-top:12px">
          <button class="btn sm primary" id="up-add">＋ 家族を追加</button>
          <button class="btn sm" id="up-rename">名前を変更</button>
          ${users.length > 1 ? `<button class="btn sm danger" id="up-del">この人を削除</button>` : ""}
        </div>`;
      $$("[data-uid]", body).forEach((b) => (b.onclick = () => {
        State.switchUser(b.dataset.uid); modal.remove(); Nav.go("dashboard");
      }));
      $("#up-add", body).onclick = () => {
        const name = (prompt("追加する人の名前", "") || "").trim();
        if (!name) return;
        State.addUser(name); modal.remove(); Nav.go("profile");
        alert(`「${name}」を追加しました。プロフィールを入力してください。`);
      };
      $("#up-rename", body).onclick = () => {
        const name = (prompt("名前を変更", State.currentName()) || "").trim();
        if (!name) return;
        State.renameUser(State.data.currentUser, name); draw(); render();
      };
      const del = $("#up-del", body);
      if (del) del.onclick = () => {
        if (!confirm(`「${State.currentName()}」の記録をすべて削除します。よろしいですか？`)) return;
        State.deleteUser(State.data.currentUser); modal.remove(); render();
      };
    };
    draw();
  }
