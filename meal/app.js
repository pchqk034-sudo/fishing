...
写真に写る料理ごとに1食分の推定値を入れてください。`;
  ...
  function remainingMeals(log, date) {
    ...
    return [
      { key: "breakfast", label: "朝食", until: 10, w: 0.25 },
      { key: "lunch", label: "昼食", until: 15, w: 0.375 },
      { key: "dinner", label: "夕食", until: 24, w: 0.375 },
  ...
  function openRecipeSuggest(dateArg) {
    ...
    // 残りを、これから食べる食事の回数で配分する
    const rest = remainingMeals(log, date);
    const slots = rest.length ? rest : [{ key: "snack", label: "間食", w: 1 }];
    ...
    const draw = () => {
      ...
      body.innerHTML = `
        ...
        <p class="muted">これから食べるのは <b>${slots.map((x) => x.label).join("・")}</b> の${slots.length}回。
          配分すると <b>${s.label}は約 ${fmt(budget)}kcal</b> が目安です。</p>
