...
(function () {
  ...
  async function analyzePhoto(dataUrl, slot, modal) {
    ...
    const provider = State.data.settings.provider || "anthropic";
    if (!key) { statusEl.innerHTML = `<p class="muted">APIキー未設定のため解析できません。</p>`; return; }
...
写真に写る料理ごとに1食分の推定値を入れてください。`;
    ...
    try {
      const items = provider === "gemini"
        ...
        : await callAnthropic(key, media, b64, prompt);
  ...
  async function callAnthropic(key, media, b64, prompt) {
  ...
  function renderSettings(view) {
    ...
    const info = provider === "gemini"
      ? { name: "Google Gemini（無料枠あり）", ph: "AIza...", link: "https://aistudio.google.com/app/apikey", linkName: "Google AI Studio",
          note: "無料枠内なら課金なしで使えます（1日あたりの回数制限あり）。" }
      : { name: "Anthropic Claude（有料）", ph: "sk-ant-...", link: "https://console.anthropic.com/settings/keys", linkName: "Anthropic Console",
          note: "従量課金です。残高（クレジット）の登録が必要です。" };
    view.innerHTML = `
      <section class="card">
        <h2>設定</h2>
        <h3>写真AI解析（任意）</h3>
        ...
        <label class="block">使うAIサービス
          <select id="set-provider">
            <option value="gemini" ${provider === "gemini" ? "selected" : ""}>Google Gemini（無料枠あり）</option>
            <option value="anthropic" ${provider === "anthropic" ? "selected" : ""}>Anthropic Claude（有料）</option>
          </select>
        </label>
        <label class="block">${info.name} のAPIキー
          <input type="password" id="set-key" value="${s.apiKey || ""}" placeholder="${info.ph}">
        </label>
        <button class="btn primary" id="set-save">キーを保存</button>
        <p class="muted">${info.note} キーは <a href="${info.link}" target="_blank" rel="noopener">${info.linkName}</a> で取得できます。
          未設定でも手動記録・分析・グラフはすべて利用できます。</p>
      </section>
    ...
    $("#set-provider").onchange = (e) => {
      State.data.settings.provider = e.target.value;
      State.save(); renderSettings(view); // ラベル・リンクを切り替え
    };
    $("#set-save").onclick = () => {
      State.data.settings.provider = $("#set-provider").value;
      State.data.settings.apiKey = $("#set-key").value.trim();
      State.save(); alert("保存しました。");
    };
    $("#set-export").onclick = () => {
