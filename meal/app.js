  ...
  function renderSettings(view) {
    ...
    view.innerHTML = `
      <section class="card">
        ...
        <label class="block">APIキー
          <input type="password" id="set-key" value="${s.apiKey || ""}" placeholder="${info.ph}">
        </label>
        <button class="btn primary" id="set-save">保存する</button>
        <p class="muted">${info.note}<br>キーは <a href="${info.link}" target="_blank" rel="noopener">${info.linkName}</a> で取得できます。
    ...
    $("#set-ai").onchange = (e) => {
      applyChoice(e.target.value);
      State.save(); renderSettings(view); // 説明・プレースホルダを切り替え
    };
    $("#set-save").onclick = () => {
