...
(function () {
  ...
  function openPhotoPicker(slot) {
    ...
    body.innerHTML = `
      ...
      ${hasKey ? "" : `<p class="warn-box">⚠️ AI解析には設定画面で Claude API キーの登録が必要です。未登録の場合は「料理を選ぶ」から手動で追加してください。</p>`}`;
...
写真に写る料理ごとに1食分の推定値を入れてください。`;
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `この料理写真を管理栄養士として分析してください。${schemaHint}` },
            ],
          }],
        }),
      });
      ...
      showPhotoResult(parsed.items || [], slot, modal, statusEl, resultEl);
    } catch (e) {
  ...
  function renderSettings(view) {
    ...
    view.innerHTML = `
      <section class="card">
        <h2>設定</h2>
        <h3>写真AI解析（任意）</h3>
        <p class="muted">Claude の画像解析APIを使うと、料理写真からカロリー・食材・栄養素を自動推定できます。
          APIキーはこの端末のブラウザ内（localStorage）にのみ保存され、外部に送信されるのは解析時のAnthropic APIへの通信のみです。</p>
        <label class="block">Claude API キー
          <input type="password" id="set-key" value="${s.apiKey || ""}" placeholder="sk-ant-...">
        </label>
        <button class="btn primary" id="set-save">キーを保存</button>
        <p class="muted">キーは <a href="https://console.anthropic.com/" target="_blank" rel="noopener">Anthropic Console</a> で取得できます。未設定でも手動記録・分析・グラフはすべて利用できます。</p>
      </section>
