...
写真に写る料理ごとに1食分の推定値を入れてください。`;
  ...
  function renderDashboard(view) {
    ...
    $("#dash-recipe").onclick = () => openRecipeSuggest();
      ...
      try {
        const answer = await askAI(
          [{ role: "user", content: `${buildContext()}\n\n【質問】${text}` }].concat(
            (State.data.chat || []).slice(-6, -1).map((m) => ({ role: m.role, content: m.content }))),
          AI_SYSTEM);
  ...
  function openRecipeSuggest() {
    ...
    const combos = suggestCombos(remain, needP, needFiber);

    const modal = makeModal("🍽 残りカロリーでレシピ提案");
    const body = $(".modal-body", modal);
    body.innerHTML = `
      <p class="muted">残り <b>${fmt(remain)}kcal</b>／不足たんぱく質 <b>${fmt(needP)}g</b>／不足食物繊維 <b>${fmt(needFiber)}g</b> に合う組み合わせです。</p>
      ${remain < 150 ? `<p class="warn-box">目標までの残りが少ないため、汁物やサラダなど軽いものが安心です。</p>` : ""}
      ${combos.length ? combos.map((c, i) => `
        <div class="ph-card">
          ...
          <div class="row wrap">
            <button class="btn sm primary" data-add-combo="${i}">この組み合わせを記録</button>
          </div>
    ...
    $$("[data-add-combo]", body).forEach((btn) => (btn.onclick = () => {
      ...
      const slotKey = prompt("どの食事に記録しますか？ 1:朝食 2:昼食 3:夕食 4:間食", "3");
      const map = { "1": "breakfast", "2": "lunch", "3": "dinner", "4": "snack" };
      const key = map[String(slotKey || "").trim()];
      if (!key) return;
      c.items.forEach((f) => State.log(todayStr()).meals[key].push({ ...f, qty: 1, at: Date.now() }));
    ...
    $("#rc-ai", body).onclick = async () => {
      ...
      try {
        const ans = await askAI([{ role: "user", content: `${buildContext()}\n\n`
          + `【依頼】残り${fmt(remain)}kcal以内で、不足しているたんぱく質${fmt(needP)}gと食物繊維${fmt(needFiber)}gを補える献立を2案。`
